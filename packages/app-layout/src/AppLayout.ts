import { CSSResult, css, html, svg, LitElement, TemplateResult, PropertyValues } from 'lit';
...
function saveTheme(theme: string | undefined) {
  if (!theme) {
    localStorage.removeItem(`${AppLayout.is}.theme`);
    return;
  }
  localStorage.setItem(`${AppLayout.is}.theme`, theme);
}

function loadTheme(): string | undefined {
  const result = localStorage.getItem(`${AppLayout.is}.theme`);
  return !result?.length ? undefined : result;
}

export class AppLayout extends LitElement {
  @property({ type: Boolean, attribute: 'randomize-brand-color' }) randomizeBrandColor: string | undefined = undefined;
  @state() randomBrandColor: string | undefined = undefined;
  @property({ type: String, reflect: true, attribute: 'brand-color' }) brandColor: string | undefined = undefined;
  @property({ type: String, reflect: true }) theme: string | undefined = undefined;
  @property({ type: Boolean }) noPersistTheme: boolean = false;
  ...
  private renderBrandStyle(): TemplateResult {
    const color = this.randomBrandColor ?? this.brandColor;
    if (!color) {
      return html``; //nothing
    }
    const brandColorCss = rebrand(color);
    return html`<style>
      ${brandColorCss}
    </style>`;
  }
  ...
  protected willUpdate(props: PropertyValues<this>): void {
    this.shouldComputeRandomBrandColor(props);
    if (!this.hasUpdated && !this.noPersistTheme) {
      this.theme = loadTheme();
    }
  }

  private shouldComputeRandomBrandColor(props: PropertyValues<this>) {
    if (!props.has('randomizeBrandColor')) {
      return;
    }
    if (this.randomizeBrandColor) {
      this.randomBrandColor = colorNames[Math.floor(Math.random() * colorNames.length)];
    } else {
      this.randomBrandColor = undefined;
    }
  }
